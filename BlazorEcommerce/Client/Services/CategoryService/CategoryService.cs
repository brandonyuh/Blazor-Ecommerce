namespace BlazorEcommerce.Client.Services.CategoryService
{
	public class CategoryService : ICategoryService
	{
		...
		{
			var response = await _http.PostAsJsonAsync<Category>("api/Category/admin", category);
		...
		{
			var response = await _http.PutAsJsonAsync<Category>("api/Category/admin", category);
			AdminCategories = (await response.Content.ReadFromJsonAsync<ServiceResponse<List<Category>>>()).Data;
			await GetCategories();
			OnChange.Invoke();
		}
